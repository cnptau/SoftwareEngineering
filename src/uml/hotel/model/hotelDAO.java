 ...
 * hotel entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see uml.hotel.model.hotel
 * @author MyEclipse Persistence Tools
 */
public class hotelDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(hotelDAO.class);
	// property constants
	...
	public hotel findById(java.lang.Integer id) {
		...
		try {
			hotel instance = (hotel) getSession().get(
					"uml.hotel.model.hotel", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(hotel instance) {
		...
		try {
			List results = getSession()
					.createCriteria("uml.hotel.model.hotel")
	...
	public hotel merge(hotel detachedInstance) {
		...
		try {
			hotel result = (hotel) getSession().merge(
					detachedInstance);
